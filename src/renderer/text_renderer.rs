...
impl Renderer<PersonalInfo, String> for TextRenderer {
    fn render(self: &Self, element: &PersonalInfo, config: &Config) -> Result<String, String> {
        let text = right_and_left_aligned(&element.github,
                                          &element.email,
...
impl Renderer<Objective, String> for TextRenderer {
    fn render(self: &Self, element: &Objective, config: &Config) -> Result<String, String> {
        let text = split_string_across_lines(&element.objective,
                                             config.format_config.text_config.width,
                                             None,
                                             None);
        Ok(text)
...
impl Renderer<Vec<ProfessionalExperience>, String> for TextRenderer {
    fn render(self: &Self, element: &Vec<ProfessionalExperience>, config: &Config) -> Result<String, String> {
        ...
        let exp = element.iter()
            ...
            .reduce(|a, b| {
                Ok(format!("{}\n\n{}", a?, b?))
            }).unwrap_or(Err(format!("An error occurred while rendering professional experience to plain text.")))?;
...
impl Renderer<OtherExperience, String> for TextRenderer {
    fn render(self: &Self, element: &OtherExperience, config: &Config) -> Result<String, String> {
        ...
        let projects = element.projects.iter()
            .map(|s| {
                self.render(s, config)
            })
            .reduce(|a, b| {
                Ok(format!("{}\n{}", a?, b?))
            })
            .unwrap_or(Err(format!("An error occurred while rendering other experience to plain text.")))?;
...
impl Renderer<ProjectInfo, String> for TextRenderer {
    fn render(self: &Self, element: &ProjectInfo, config: &Config) -> Result<String, String> {
        Ok(split_string_across_lines(&element.description,
                                     config.format_config.text_config.width,
                                     Some(String::from("- ")),
                                     Some(String::from("  "))))
    }
...
mod test {
    ...
    fn test_personal_info_text_renderer() {
        ...
        assert_eq!(rendered, "github.com/foo                         foo@bar.com");
    }
}
